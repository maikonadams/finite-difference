...
int main()
{
   ...
   rz=(D12*Dt)/(Dz*Dz);
   ru2=(u*Dt)/Dz;
   ru3=((u*Dt)*(1.0-((double)M*M*Dr*Dr)/(a*a)))/Dz;
  ...
  for(k=1;k<=(N-1);k++)
   for(i=1;i<=(M-1);i++){
    ...
    if(n==0) {
    ru=Dt*u*(1-((double)i*i*Dr*Dr)/(a*a))/(Dz);
    C[i][0][n+1]=C[i][0][n]+rr*(C[i+1][0][n]-2*C[i][0][n]+C[i-1][0][n])
                 +(rr/(2.0*(double)i))*(C[i+1][0][n]-C[i-1][0][n])+rz*(C[i][1][n]-C[i][0][n])
                 -ru*(C[i][1][n]-C[i][0][n]);

    C[0][k][n+1]=4.0*rr*(C[1][k][n]-C[0][k][n])+rz*(C[0][k+1][n]-2.0*C[0][k][n]+C[0][k-1][n])
                 +C[0][k][n]-ru2*(C[0][k+1][n]-C[0][k-1][n]);

    C[i][k][n+1]=C[i][k][n]+rr*(C[i+1][k][n]-2.0*C[i][k][n]+C[i-1][k][n])+
                 (double)(rr/(2.0*i))*(C[i+1][k][n]-C[i-1][k][n])+
                 rz*(C[i][k+1][n]-2.0*C[i][k][n]+C[i][k-1][n])-
                 ru*(C[i][k+1][n]-C[i][k-1][n]);
    C[M][k][n+1]=2.0*rr*(C[M-1][k][n]-C[M][k][n])+rz*(C[M][k+1][n]-2.0*C[M][k][n]+C[M][k-1][n])
...
if((fp=fopen("dadosDFFFinal.dat","w"))==NULL)
