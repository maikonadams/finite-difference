 ...
 for(n=0;n<=(tempo-1);n++){
  ...
  for(k=1;k<=(N-1);k++) {
    ...
    C[i][0][n+1]=((0.5*rr*(C[i+1][0][n+1]+C[i-1][0][n+1]+C[i+1][0][n]-2.0*C[i][0][n]+C[i-1][0][n])
                   +((rr)/((double)4.0*i))*(C[i+1][0][n+1]-C[i-1][0][n+1]+C[i+1][0][n]-C[i-1][0][n])
    ...
    C[0][k][n+1]=((2.0*rr*(C[1][k][n+1]+C[1][k][n]-C[0][k][n])
                   ...
                   -ru2*(C[0][k+1][n+1]-C[0][k-1][n+1]+C[0][k+1][n]-C[0][k+1][n]))/(2.0*rr+rz+rmax*rmax*zmax*zmax));
...
if((fp=fopen("simulacao12000.txt","w"))==NULL)
