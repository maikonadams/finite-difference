/* O problema eh o seguinte : D^2T/Dx^2=DT/Dt para 0<x<1 , t>0
            Fronteira Inicial DT=5 para x=0, t>0
            Fronteira Final   DT=10 para x=1, t>0
            Condição Inicial  T=x^2 para t=0, 0<=x<=1.
     ...
     i=3;// for(i=0;i<=M;i++)
      {
         ...
         {
            ta=ta+( (4)*( pow(-1,nu)*exp(-nu*nu*3.14*3.14*a*n*Dt)*cos(nu*3.14*i*Dx))/(nu*nu*3.14*3.14)  );
         }
         ta = 5 + 5*Dx*i +4*ta;
