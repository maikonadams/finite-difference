  ...
  for(k=1;k<=(N-1);k++)
   ...
   /* }
    else 
    {
       ...
       ru2=(u*Dt)/Dz;
       ru3=((u*Dt)*(1-(M*M*Dr*Dr)/(a*a)))/Dz;
       C[i][0][n+1]=((2*rr*(C[i+1][0][n]-C[i][0][n-1]+C[i-1][0][n])
                      +(rr/i)*(C[i+1][0][n]-C[i-1][0][n])
                      +2*rz*(C[i][1][n]-C[i][0][n-1]+C[i][0][n])
                      +C[i][0][n-1]-2*ru*(C[i][1][n]-C[i][0][n]) )/(2.0*rr+2.0*rz+1.0));

       C[0][k][n+1]=(4*rr*(2*C[1][k][n]-C[0][k][n-1])
                     +2*rz*(C[0][k+1][n]-C[0][k][n-1]+C[0][k-1][n])
                     +C[0][k][n-1]-2*ru2*(C[0][k+1][n]-C[0][k-1][n]) )/(4*rr+2.0*rz+1.0);

       C[i][k][n+1]=((2*rr*(C[i+1][k][n]-C[i][k][n-1]+C[i-1][k][n])
                      +(rr/i)*(C[i+1][k][n]-C[i-1][k][n])
                      +2*rz*(C[i][k+1][n]-C[i][k][n+1]+C[i][k-1][n])
                      +C[i][k][n-1]-2*ru*(C[i][k+1][n]-C[i][k-1][n]))/(2.0*rr+2.0*rz+1.0));

       C[M][k][n+1]= (2*rr*(2*C[M-1][k][n]-C[M][k][n-1])
                     +2*rz*(C[M][k+1][n]-C[M][k][n-1]+C[M][k-1][n])
                     +C[M][k][n-1]-2*ru3*(C[M][k+1][n]-C[M][k-1][n]) )/(2.0*rr+2.0*rz+1.0);
    }    */
   }


if((fp=fopen("DadosExplícito.txt","w"))==NULL)
        printf("Erro na criação de dados.txt");

   fprintf(fp,"Explicito Du Fort-Frankel\n");


   for(n=0;n<=tempo;n+=50){
     for(i=0;i<=M;i+=2){
       k=(N-4);
