...
while(e>0.00001){
   for(n=0;n<=(tempo-1);n++){
      temp=(T[n][0] +r*(1+p)*(T[n][1]-T[n][0]+T[n+1][1]))/(1+r);
      while(fabs(temp-T[n+1][0])>max){
      ...
      for(i=1;i<=(M-1);i++)
      {
         temp=(T[n][i]+(r/2)*(T[n+1][i-1]+T[n+1][i+1]+T[n][i-1]-2*T[n][i]+T[n][i+1]+p/(2*i)*(T[n+1][i+1]-T[n+1][i-1]+T[n][i+1]-T[n][i-1])))/(1+r);
